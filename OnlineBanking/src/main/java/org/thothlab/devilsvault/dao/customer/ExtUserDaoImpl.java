...
public class ExtUserDaoImpl{
    ...
    {
        String query = "INSERT INTO external_user ( id , name , address ,city,state,country,pincode, phone , email , date_of_birth , ssn ) VALUES (?,?,?,?,?,?,?,?,?,?,?)";
        Connection con = null;
        PreparedStatement ps = null;
        try{
            con = dataSource.getConnection();
            ...
            if(out !=0){
                 String queryID = "SELECT id from external_user where email= '" + userdetails.getEmail() + "'"; 
